...
export type ProductGetListOptions<E extends Embed[]> = {
  embed?: E
  shop_no?: number,
...
export interface IconShowPeriod {
  start_date: string,
  end_date: string,
}
...
export interface DecorationImages {
  use_show_date: "T" | "F",
...
export type Embed = 'variants' | 'options' | 'memos' | 'hits' | 'seo' | 'tags' | 'additionalimages' | 'decorationimages' | 'discountprice'// | 'benefits';
...
export type EmbedIncludes<E extends Embed[]> = UnionToIntersection<
  {
    [K in E[number]]: K extends keyof EmbedFields ? EmbedFields[K] : {}
  }[E[number]]
>

export type ProductWithEmbed<E extends Embed[]> = Product & EmbedIncludes<E>
...
export interface DeleteProductRequest {
  ...
  product_no: number; // Required
}

export interface DeleteProductResponse {
  product: {
    product_no: number; // The ID of the deleted product
  };
}
