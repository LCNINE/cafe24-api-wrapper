...
export class ProductService extends AbstractService {
  ...
  public async getOne<E extends Embed[]>(
    productNo: number,
   ...
   * 상품 재고 조회
   */
  public async getInventory(productNo: number) {
    const url = `/products/${productNo}/inventories`;
    const res = await this.client.get<{ inventories: ProductInventory[] }>(url);
   ...
   * 상품 옵션 조회
   */
  public async getOptions(productNo: number) {
    const url = `/products/${productNo}/options`;
    const res = await this.client.get<{ options: ProductDetailOption }>(url);
   ...
   * 상품 품절 여부 조회
   */
  public async getSoldoutStatus(productNo: number) {
    const url = `/products/${productNo}/soldout`;
    const res = await this.client.get<{ soldout: ProductSoldout }>(url);
